...
class ClientForm(forms.ModelForm):
    class Meta:
        model = Client
        fields = [ 'type', 'tva', 'clientName', 'adresse', 'zip_code', 'country', 'contact', 'email', 'phone_number', 'website']
    ...
    class Meta:
        ...
        widgets = {
            'client': AutocompleteSelect(
                Invoice._meta.get_field('client').remote_field,
                admin.site,
...
#     def __init__(self,*args,**kwargs):
#         self.initial_product = kwargs.pop('initial_product')
#         self.PRODUCT_LIST = Product.objects.all()
#         self.PRODUCT_CHOICES = [('-----', '--Select a Product--')]


#         for products in self.PRODUCT_LIST:
#             d_t = (products.uniqueId, products.title)
...
#                                         label='Choose a related product',
