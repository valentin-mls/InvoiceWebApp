...
from .forms import DateInput,UserLoginForm,ClientForm,ProductForm,InvoiceForm,SettingsForm,ClientSelectForm
from .models import Settings,Product,Invoice,Client
from .functions import emailInvoiceClient
...
def deleteProducts(request, slug):
    try:
...
def editClient(request, pk, model, cls):
    ...
    if request.method == "POST":
        ...
        return render(request, 'invoice/edit-client.html', {'form': form})


def edit_client(request, pk):
    return editClient(request, pk, Client, ClientForm)
...
def createBuildInvoice(request, slug):
    ...
    if request.method == 'GET':
        ...
        # prodSelect_form = ProductSelectForm(initial_product=invoice.product)
        context['prod_form'] = prod_form
        context['inv_form'] = inv_form
        context['client_form'] = client_form
        # context['prodSelect_form'] = prodSelect_form
    ...
    if request.method == 'POST':
        ...
        # prodSelect_form = ProductSelectForm(request.POST, initial_product=invoice.product, instance=invoice)
        ...
        if prod_form.is_valid():
            ...
            return redirect('create-build-invoice', slug=slug)
        # elif prodSelect_form.is_valid() and 'products' in request.POST:

        #     prodSelect_form.save()
        #     messages.success(request, "Product added to invoice succesfully")
        #     return redirect('create-build-invoice', slug=slug)
        else:
            context['prod_form'] = prod_form
            context['inv_form'] = inv_form
...
def viewPDFInvoice(request, slug):
    ...
    #Get Client Settings
    p_settings = Settings.objects.get(clientName='Les VRAIS')
...
def companySettings(request):
    company = Settings.objects.get(clientName='Les VRAIS')
